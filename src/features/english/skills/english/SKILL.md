...
- **read**: en を **General American の自然な会話速度**で読んだ「圧縮カナ」。日本語の外来語読みや
  単語ごとの音写にはしない。まず文全体で強勢・弱形・連結・同化・未破裂・脱落を決め、最後に一度だけ
  カナへ落とす（`swap in a` → `スワ·ピ·ナ`、`route handlers` → `ルーｯ ハン·ドラーｽﾞ`）。
  次の記法を一貫して使う:
  - **全角カナ**は、はっきり聞こえる音節の核。**半角カナ**は、短く弱い音節または独立した日本語の
    モーラにしてはいけない子音成分（`キープ` ではなく `キーﾌﾟ`、弱形の `as` は `ｧｽﾞ`）。
    半角を装飾には使わない。
  - **半角 `ｯ`** は、後続子音の前で閉鎖だけ作り破裂させない stop。音自体が脱落する場合は書かず、
    必要なら memo で説明する（`just keep` → `ジャｽｯ キーﾌﾟ`）。
  - 英語にない母音を子音へ足さない。dark l は直前母音へ吸収して `ル` を立てず、語末の /ŋ/ に
    `グ` を足さない（`everything` → `エｳﾞ·リ·スィン`）。
  - **`·` と半角スペースはどちらも音節境界**。`·` は同じ息のまとまり、スペースはリズムの切れ目。
    両方で区切った単位数を en の発話上の音節数と一致させる。綴りの単語境界は維持しなくてよい。
  - 機能語の弱母音も隠さない。連結後の実音を 1 本で書く
    （`else as is` → `エｩｽ·ｧｽﾞ·ィｽﾞ`、文全体なら
    `Just keep everything else as is.` → `ジャｽｯ キーﾌﾟ エｳﾞ·リ·スィン エｩｽ·ｧｽﾞ·ィｽﾞ`）。
  半角カナは厳密な音声記号ではなく、**日本語話者が余分な母音を発音しないための強弱指定**として使う。
- **memo**: read で起きている**連結・音変化の解説**。**1 点 1 文の文字列配列**で 1〜2 点。
  1 要素に複数の現象を詰めない — 現象が 2 つなら要素も 2 つに割る。Discord では 1 点 1 bullet で出る。
  例: `["just の /t/ は /k/ の前で破裂させず、ジャｽｯ と止める。", "else as is は弱形 /əz/ と連結し、エｩｽ·ｧｽﾞ·ィｽﾞ と流れる。"]`
  ...
  "cards": [
    {
      ...
      "read": "ファ·ナウ カｯ·ﾀ ブランﾁ ｧﾝ テイ·キｯ オーｩ ｻﾞ ウェイ ﾄｩ·ｧ ピー·アール",
      "memo": [
        "cut a は /t/ が母音に連結して カｯ·ﾀ。",
        "take it は /k/ と /ɪ/ がつながって テイ·キｯ と流れる。"
      ],
